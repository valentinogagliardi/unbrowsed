...
def test_query_by_role_with_name_and_description():
    ...
    result = query_by_role(dom, "button", name="Submit", description="Submits the first form")
    assert result is not None
    
    result = query_by_role(dom, "button", name="Submit", description="Submits the second form")
    assert result is not None
    
    result = query_by_role(dom, "button", name="Submit", description="Does something else")
    assert result is None
...
def test_query_by_role_with_complex_name_and_description():
    ...
    assert query_by_role(dom, "textbox", description="Enter your username or email")
    assert query_by_role(dom, "textbox", description="Must be at least 8 characters")
    assert query_by_role(dom, "textbox", name="Username", description="Enter your username or email")
    assert query_by_role(dom, "textbox", name="Password", description="Must be at least 8 characters")
    assert not query_by_role(dom, "textbox", name="Username", description="Must be at least 8 characters")
    assert not query_by_role(dom, "textbox", name="Password", description="Enter your username or email")
